  ...
  const fetchNotifications = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to fetch admin notifications.');
      }
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button onClick={fetchNotifications} variant="outline" size="icon" disabled={isLoading}>
      ...
      <CardContent className="space-y-4">
        {isLoading ? (
          ...
          notifications.map((notif) => (
            <div key={notif.id} className="flex items-start gap-4 p-4 border rounded-lg bg-muted/30">
