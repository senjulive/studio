...
import { Loader2, PlusCircle, Gift, Image as ImageIcon, Trash2, Edit } from "lucide-react";
...
export function PromotionManager() {
  ...
  const fetchPromotions = React.useCallback(async () => {
    ...
    try {
      ...
      setPromotions(data.sort((a: Promotion,b: Promotion) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()));
    } catch (error: any) {
  ...
  const handleDelete = async (promotionId: string) => {
    ...
    try {
      ...
      toast({ title: "Promotion Deleted", description: "The promotion has been successfully removed." });
  ...
  const onSubmit = async (values: PromotionFormValues) => {
    if (!adminPassword) {
      toast({ title: 'Auth Error', description: 'Admin password not found.', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      const endpoint = editingPromotion ? `/api/admin/promotions?id=${editingPromotion.id}` : '/api/admin/promotions';
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Failed to save promotion.');

      toast({ title: `Promotion ${editingPromotion ? 'Updated' : 'Created'}`, description: "The promotion has been saved." });
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>Create and manage promotions visible to all users.</CardDescription>
        </div>
            ...
            {isLoading ? (
              ...
              promotions.map((promo) => (
                <TableRow key={promo.id}>
                  <TableCell>
                    {promo.image_url ? (
                      <Image src={promo.image_url} alt={promo.title} width={64} height={40} className="rounded-md object-cover" />
                    ) : (
                  ...
                  <TableCell>
                    <Badge variant={
                      ...
                      promo.status === 'Upcoming' ? 'secondary' :
                      'outline'
                    }>{promo.status}</Badge>
                  </TableCell>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="e.g., Summer Trading Bonus" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Describe the promotion..." {...field} rows={4} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
