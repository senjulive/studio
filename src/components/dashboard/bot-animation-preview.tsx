...
export function BotAnimationPreview() {
  return (
    <div className="w-full h-16 flex justify-center items-end gap-1.5" aria-hidden="true">
      <div
        className={cn(
          "w-2.5 bg-primary/30 rounded-t-sm animate-bar-pulse origin-bottom"
        )}
        style={{ height: '60%', animationDelay: '0s' }}
      />
      <div
        className={cn(
          "w-2.5 bg-primary/50 rounded-t-sm animate-bar-pulse origin-bottom"
        )}
        style={{ height: '80%', animationDelay: '0.2s' }}
      />
      <div
        className={cn(
          "w-2.5 bg-primary/70 rounded-t-sm animate-bar-pulse origin-bottom"
        )}
